...
def run_demo_trader(
    ...
    opened = 0
    if _should_open_trade(frame, asset, plan, prices):
        frame = _append_trade(frame, asset, plan, prices, source)
        opened = 1
    _save_demo_trades(frame, path)
    return _summarize(frame, opened=opened, closed=closed)
...
def accountability_scorecard(frame: pd.DataFrame) -> dict[str, float | int]:
    if frame.empty:
        return {"total": 0, "closed": 0, "open": 0, "wins": 0, "losses": 0, "win_rate": 0.0, "expectancy_r": 0.0}
...
def _should_open_trade(frame: pd.DataFrame, asset: str, plan: IntradayTradePlan, prices: pd.DataFrame) -> bool:
    if prices.empty or plan.action not in {"WAIT_LONG_TRIGGER", "WAIT_SHORT_TRIGGER"}:
        return False
    if not frame.empty:
        open_for_asset = frame[(frame["asset"] == asset) & (frame["status"] == "open")]
        if not open_for_asset.empty:
            return False
...
def _summarize(frame: pd.DataFrame, opened: int, closed: int) -> DemoTradeSummary:
